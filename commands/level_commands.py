    ...
    async def global_rank(interaction: discord.Interaction):
        ...
        try:
            ...
            for user_id, xp, level in leaderboard:
                # Primero caché local del guild; evita llamadas extra
                member = interaction.guild.get_member(int(user_id)) if interaction.guild else None

                # Si no está en caché, intentamos API; si no existe, lo saltamos
                ...
                shown += 1
                msg += f"{shown}. {member.display_name} — {xp} XP (Level {level})\n"
            ...
            logging.exception(f"Error in /rank: {e}")
            try:
                await interaction.followup.send(
